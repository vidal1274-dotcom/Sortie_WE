...
export function generateDayPlan(sites, vehicleProfile, options = {}) {
  ...
  // 3. Nearest-neighbor depuis Uchaud
  ...
  steps.push({ time: _fmt(cur), icon: '🚗', label: 'Départ depuis Uchaud', type: 'depart' });
  ...
  totalKm += retKm;
  cur += retMin;
  steps.push({ time: _fmt(cur), icon: '🏠', label: `Retour vers Uchaud (~${Math.round(retKm)} km)`, type: 'return' });
