/* =========================================================
   BLOC 01 â€” IMPORTS ET CONSTANTES
   ========================================================= */
...
export function generateDayPlan(sites, vehicleProfile, options = {}) {
  ...
  // 1. Candidats : GPS requis + dans le rayon + non visitÃ©s
  ...
  // 4. ItinÃ©raire chronologique
  const steps = [];
  let cur = DEPART_HOUR_MIN;
  steps.push({ time: _fmt(cur), icon: 'ðŸš—', label: 'DÃ©part depuis Nages', type: 'depart' });
  ...
  selected.forEach((site, idx) => {
    ...
    steps.push({
      time: _fmt(cur), icon: 'ðŸ“',
      label: `ArrivÃ©e : ${site.destination}`,
      type: 'arrival', site,
    ...
    if (!mealInserted && cur >= 12 * 60 && cur < 14 * 60 && idx < selected.length - 1) {
      steps.push({ time: _fmt(cur), icon: 'ðŸ½ï¸', label: 'Pause repas (restaurant ou pique-nique)', type: 'meal' });
      cur += MEAL_MIN;
      mealInserted = true;
    }
  ...
  totalKm += retKm;
  cur += retMin;
  steps.push({ time: _fmt(cur), icon: 'ðŸ ', label: `Retour vers Nages (~${Math.round(retKm)} km)`, type: 'return' });
...
/* =========================================================
   BLOC 03 â€” PERSISTANCE LOCALSTORAGE
   ========================================================= */
...
export function renderDayPlan(plan) {
    ...
    return `
      <div class="dp-step dp-step-${s.type}" ${click}>
        ...
        <div class="dp-step-body">
          ...
          ${s.travelKm ? `<div class="dp-step-meta">${s.travelKm} km Â· ~${s.travelMin} min de trajet</div>` : ''}
        </div>
      </div>`;
  }).join('');

  const costStr = plan.energyCost != null
    ? `âš¡ CoÃ»t Ã©nergie estimÃ© : <strong>${formatCurrency(plan.energyCost)}</strong>`
    : 'âš™ï¸ Configurez votre vÃ©hicule pour estimer le coÃ»t';
  ...
  return `
    <div class="day-plan-wrapper">
      <div class="dp-header">
        <div class="dp-title">ðŸ“… Programme de la journÃ©e</div>
        <div class="dp-meta-row">
          <span class="dp-badge">ðŸ—ºï¸ ${plan.sites.length} Ã©tape${plan.sites.length > 1 ? 's' : ''}</span>
          <span class="dp-badge">ðŸ“ ~${plan.totalDistanceKm} km</span>
          <span class="dp-badge">â± ${dH}h${String(dM).padStart(2,'0')}</span>
        </div>
        <div class="dp-cost-line">${costStr}</div>
        <div class="dp-disclaimer">âš ï¸ Horaires indicatifs â€” vÃ©rifiez les horaires d'ouverture rÃ©els.</div>
      </div>
      <div class="dp-steps">${stepsHtml}</div>
      <div class="dp-actions">
        <button class="btn-primary" id="btn-dp-map">ðŸ—ºï¸ Voir sur la carte</button>
        <button class="btn-secondary" id="btn-dp-save">ðŸ’¾ Sauvegarder</button>
        <button class="btn-secondary" id="btn-dp-regen">ðŸ”„ RÃ©gÃ©nÃ©rer</button>
        <button class="btn-secondary" id="btn-dp-copy">ðŸ“‹ Copier</button>
        <button class="btn-secondary dp-btn-delete" id="btn-dp-delete">ðŸ—‘ï¸ Supprimer</button>
      </div>
    </div>`;
}

/* =========================================================
   BLOC 05 â€” EXPORT TEXTE
   ========================================================= */
export function exportPlanAsText(plan) {
  const lines = [
    `Programme TREKKO â€” ${new Date(plan.generatedAt).toLocaleDateString('fr-FR')}`, ''
  ];
  plan.steps.forEach(s => lines.push(`${s.time}  ${s.icon}  ${s.label}`));
  lines.push('', `Distance totale : ~${plan.totalDistanceKm} km`);
  if (plan.energyCost != null) lines.push(`CoÃ»t Ã©nergie estimÃ© : ${plan.energyCost.toFixed(2)} â‚¬`);
  lines.push('âš ï¸ Horaires indicatifs.');
  return lines.join('\n');
}
