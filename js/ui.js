...
function buildSiteCard(site, vehicleProfile) {
  ...
  const distStr = site.distance_km ? `${site.distance_km} km` : '—';
  const budgetStr = site.budget_indicatif ? site.budget_indicatif.substring(0, 60) : '—';
  let energyStr = '';
  if (site._energy_cost != null) energyStr = `⚡ Trajet ~${site._energy_cost.toFixed(1)} €`;

  const ecoScore = site.eco_score != null
    ? `<span class="eco-score">🟢 ${site.eco_score}/100</span>`
    : '';

  const card = createElement('div', 'site-card', `
    <div class="site-card-header">
      <span class="site-name">${site.destination || site.nom || 'Site'}</span>
      <span class="site-distance">${distStr}</span>
    </div>
    <div class="site-sector">${site.secteur || site.type_sortie || ''}</div>
    <div class="site-badges">${badges}</div>
    ${site.programme_court ? `<div class="site-summary">${site.programme_court.substring(0, 100)}…</div>` : ''}
    <div class="site-footer">
      <span class="site-budget">${budgetStr}</span>
